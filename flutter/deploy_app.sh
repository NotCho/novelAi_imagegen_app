...
print_usage() {
  cat <<'EOF'
Usage: ./deploy_app.sh [options]

Options:
  --apk <path>          업로드할 APK 경로 (기본값: build/app/outputs/flutter-apk/app-release.apk)
...
필수 도구: firebase-tools, gcloud, jq, curl
인증: firebase login, gcloud auth application-default login
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --apk)
      shift
      [[ $# -gt 0 ]] || { echo "--apk 플래그에는 APK 경로가 필요합니다" >&2; exit 1; }
      APK_PATH="$1"
      ;;
    --version)
      shift
      [[ $# -gt 0 ]] || { echo "--version 플래그에는 버전 값이 필요합니다" >&2; exit 1; }
      APP_VERSION="$1"
      ;;
    --notes)
      shift
      [[ $# -gt 0 ]] || { echo "--notes 플래그에는 파일 경로가 필요합니다" >&2; exit 1; }
      NOTES_FILE="$1"
      ;;
    --notes-text)
      shift
...
# ------------------------------
if [[ ! -f "$APK_PATH" ]]; then
  echo "APK 파일을 찾을 수 없습니다: $APK_PATH" >&2
  exit 1
fi

if [[ -z "$APP_VERSION" ]]; then
  if ! APP_VERSION="$(grep '^version:' pubspec.yaml | awk '{print $2}')"; then
    echo "pubspec.yaml에서 버전 정보를 찾지 못했습니다. --version 옵션을 사용하여 직접 지정하세요." >&2
    exit 1
  fi
fi

if [[ -z "$APP_VERSION" ]]; then
  echo "Remote Config에 설정할 버전을 결정할 수 없습니다." >&2
  exit 1
fi
...
HTTP_STATUS=$(curl -s -o /tmp/deploy_remote_config_response.json -w "%{http_code}" \
...
if [[ "$HTTP_STATUS" != "200" ]]; then
  ...
  cat /tmp/deploy_remote_config_response.json >&2
  exit 1
fi
